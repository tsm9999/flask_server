...
df = pd.read_excel("C:/Users/TANUSH/Desktop/sih_docs/SIH Data/medicines.xlsx",sheet_name="Sheet1",header=None)
df.head()
...
entity_tagger=StanfordNERTagger('E:/sih_flask/v7_ner_model.ser.gz', 'E:/sih_flask/stanford-ner-3.9.2.jar')
#print(tokens)
filtered_sentence=[]
for w in tokens:
  ...
  if pair[1]=='T':
    ...
    while pair[1]!='TIM' and pair[1]!='DAY' and pair[1]!= "T":
      if pair[1]=='TYP':
        ...
        search_med+=pair[0]+' '
        print(search_med)
        medtuple[index]= get_close_matches(search_med,medicine_list,n=5,cutoff=0.5)
        i=i+1
        if i >len(tagged_entity)-1:
          break
        pair=tagged_entity[i]
